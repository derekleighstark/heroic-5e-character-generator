...
const base = Function(`${appPrefix}; return {classes, origins, talents, merits, flaws};`)();
...
const iconFor = type => ({ talent: "icons/svg/upgrade.svg", merit: "icons/svg/aura.svg", flaw: "icons/svg/downgrade.svg", origin: "icons/svg/mystery-man.svg", class: "icons/svg/angel.svg", power: "icons/svg/explosion.svg" })[type] ?? "icons/svg/item-bag.svg";
...
const merits = Object.entries(rules.meritRules).map(([name, text]) => item("merit", name, { description: html(text), category: /Origin Merit/.test(text) ? "Origin Merit" : "Merit", tags: (String(text).match(/^Rating [^.]+/) || [""])[0] }));
const flaws = Object.entries(rules.flawRules).map(([name, text]) => item("flaw", name, { description: html(text), category: /Origin Flaw/.test(text) ? "Origin Flaw" : "Flaw", tags: (String(text).match(/^Rating [^.]+/) || [""])[0] }));
...
const counts = {};
for (const [name, documents] of Object.entries({ talents, merits, flaws, powers, origins, classes })) counts[name] = await writePack(name, documents);
